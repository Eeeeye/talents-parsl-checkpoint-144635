...
from parsl.dataflow.completion import TaskOutcome
            ...
            if task_record['from_memo']:
                ...
                if not task_record['join']:
                    ...
                    if isinstance(joinable, Future):
                        ...
                        invalid_join = TypeError(
                            f"join_app body must return a Future or list of Futures, got {joinable} of type {type(joinable)}"
                        )
                        task_record['time_returned'] = datetime.datetime.now()
                        self.update_task_state(task_record, States.failed)
                        task_record['time_returned'] = datetime.datetime.now()
