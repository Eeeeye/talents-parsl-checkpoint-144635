...
The next run also failed to discover checkpoints written below the configured run
root.
...
3. **Deferred checkpoint modes retain completed outcomes.** In `manual`,
...
4. **Automatic recovery spans numbered runs.** If `checkpoint_files` is omitted
   and checkpointing is enabled, a new DFK must discover valid checkpoints under
   previous numbered run directories of `Config.run_dir`. It must not search only
   the newly created per-run directory. The first invocation in a fresh run root
   executes normally; the same cached invocation in a second Parsl process must
   load the previous result, set `from_memo` to true, and not execute the app body.

5. **Explicit recovery remains supported.** A non-empty
   `Config(checkpoint_files=[...])` is authoritative. Each entry names a checkpoint
   directory containing `tasks.pkl`; it must still be loaded even if it lies
   outside `Config.run_dir`.

6. **Failures are not successful cache entries.** A failed app must complete with
...
7. **Invalid checkpoints fail closed.** A requested checkpoint that is missing,
   truncated, not a pickle stream of records, or has an invalid record schema must
   raise `parsl.dataflow.errors.BadCheckpoint` while loading. Do not silently
   discard a corrupt suffix, repair the file, or continue with a partial cache.
   The invalid file must remain byte-for-byte unchanged.

## Checkpoint compatibility contract

`tasks.pkl` is an append-only sequence of Python pickle objects. Every accepted
record is exactly a dictionary with the keys:
...
task, however, the memoized result and any required `task_exit` record must be
published before its user-facing Future is published.
...
- Preserve `Config(checkpoint_mode=..., checkpoint_files=..., run_dir=...)`,
  ...
  `task_record["from_memo"]`, and the on-disk schema above.
...
recovery, deferred modes, concurrent completion, failed apps, malformed
checkpoints, and repeat execution.
